...
const COL_L_TARGET = 2;             // middle left colon
const COL_F_TARGET = 3;             // middle right colon
const COL_L_START  = 0;             // leader spawn colon
const COL_F_START  = 5;             // follower spawn colon
...
/* ---------- helper ---------- */
function cell(c,r){ return { left:c*DEFAULT_CELL, top:r*DEFAULT_CELL }; }
...
export function initDanceGame(mountId='decision-tool'){
  ...
  const nav = () => { prevBtn.disabled = ptr<=0; nextBtn.disabled = ptr>=history.length-1; };

  /* ---------- builders ---------- */
  const buildMatrix = () => Array.from({length:pairCount},
  ...
  selAlgo.onchange = ()=>{
    const need = !NO_ITER.has(selAlgo.value);
    selIter.disabled = !need;
    selIter.classList.toggle('opacity-50', !need);
  };
  selAlgo.onchange();
  ...
  /* ---------- setup panel ---------- */
  function line(d,isL,idx){ return `
  ...
  function renderSetup(){
    ...
    /* real-time style change when its selected from dropbox */
    setupBox.querySelectorAll('select').forEach(sel=>{
      sel.onchange = ()=>{
        const d = sel.dataset.kind==='L' ? leaders[+sel.dataset.idx]
                                         : followers[+sel.dataset.idx];
        d.style = sel.value;
    ...
    setupBox.querySelectorAll('input[type="range"]').forEach(r=>{
      ...
      r.oninput = ()=>{
        ...
        const d = r.dataset.kind==='L' ? leaders[r.dataset.idx] : followers[r.dataset.idx];
        d.skill = +r.value;
      };
    });
  }
  ...
  function spawnGrid(){
    ...
    grid.style.height = `${pairCount*DEFAULT_CELL}px`;
    const scale = Math.min(1, grid.parentElement.clientWidth/fullW);
    ...
    [...leaders,...followers].forEach((d,i)=>{
      ...
      wrap.style = `position:absolute;left:${pos.left}px;top:${pos.top}px;width:${DEFAULT_CELL}px;height:${DEFAULT_CELL}px;transition:left 1.2s,top 1.2s`;
      wrap.innerHTML = `
        <div class="text-center text-xs font-semibold">${d.name}</div>
        <div id="sprite-${d.name}" class="dance-sprite ${d.style} w-16 h-16 mx-auto"
             style="background-image:url(${d.sprite})"></div>`;
  ...
  function drawMatrix(){
    matTbl.innerHTML =
      ...
      matrix.map((row,i)=>`<tr><th>${leaders[i].name}</th>`+
        row.map(s=>`<td>${s}</td>`).join('')+'</tr>').join('');
  }
  ...
  function narrative(p){
    ...
    const skillTxt = gap<=5 ? 'Skills match perfectly.'
                    ...
                    : 'Big skill gap.';
    return `• <strong>${p.l.name}</strong> & <strong>${p.f.name}</strong> — <em>${p.score}</em> pts<br>${styleTxt} ${skillTxt}`;
  }

  /* ---------- animate snapshot ---------- */
  async function animateSnap(s){
    s.pairs.forEach((p,i)=>{
      const l = grid.querySelector('#sprite-'+p.l.name).parentElement;
      const f = grid.querySelector('#sprite-'+p.f.name).parentElement;
      const posL = cell(COL_L_TARGET,i), posF = cell(COL_F_TARGET,i);
      l.style.left=`${posL.left}px`; l.style.top=`${posL.top}px`;
      f.style.left=`${posF.left}px`; f.style.top=`${posF.top}px`;
    });
  ...
  async function runAlgo(algo,iters){
    history.length = 0; ptr = -1; nav(); 
    spawnGrid();
    /* Don't delete here since it ensures the first animation is not skipped */
    await new Promise(r =>
  ...
  function template(){
    ...
    return `
<div class="min-h-[80vh] flex flex-col items-center py-8 bg-gradient-to-br from-slate-50 to-indigo-100">
  <div class="bg-white/70 backdrop-blur-md rounded-2xl p-6 shadow-xl w-full max-w-4xl flex flex-col gap-6">
    ...
    <div class="flex flex-wrap gap-3 justify-center items-center">
      ...
      <label class="flex items-center gap-1">Iter
        <select id="iter" class="border rounded px-1 py-0.5">
          ${ITER_OPTS.map(v=>`<option${v===5_000?' selected':''}>${v}</option>`).join('')}
        </select>
      </label>
    ...
    <div class="w-full overflow-x-auto"><div id="grid" class="relative mx-auto"></div></div>
